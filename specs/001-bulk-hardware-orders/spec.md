...
- An enterprise client submits an order with no line items: the system rejects the submission (see FR-015). An order whose calculated Net Total is zero or negative is blocked from finalization using the same rejection pattern as invalid contract terms (see FR-004, FR-026).
...
- **FR-026**: System MUST block an order from being finalized, using the same rejection pattern as FR-004, whenever its calculated Net Total (per FR-003) is zero or negative.
