...
- **Dashboard page**: Active Orders & Lifecycle Tracking (one card per Active order, each showing a 5-step lifecycle indicator and a cancel control), the Order History Log (past orders with a status indicator and full lifecycle transition timestamp history per entry), and an action to start a new order that navigates to the order detail page.
...
- **SC-001**: An enterprise client submitting a valid bulk order receives a calculated Net Total in under 5 seconds; the same under-5-seconds target applies to a Line Item edit's recalculated Gross/Net Total (FR-025) and to Order History retrieval (FR-008).
