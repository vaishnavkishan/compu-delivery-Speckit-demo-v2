...
- Q: When the system calculates Gross Total and Net Total, what decimal precision and rounding rule should it use, and what should happen if a computed Net Total is zero or negative? → A: Round to 2 decimal places (standard currency cents) using round-half-up; if the resulting Net Total is zero or negative, the system blocks the order from finalization, using the same rejection pattern as FR-004's contract-terms block.
- Q: Does each Enterprise Client have exactly one Contract Discount Terms record active at any given time, or can a client have multiple discount-term records the system must pick among? → A: One active record per client — each client has a single current Contract Discount Terms record with a validity/expiration status; "current" means that one record is active and not expired at calculation time, with no selection logic among multiple records needed.
- Q: For the audit trail (lifecycle transitions and Net Total calculations), what timestamp precision is required and how long must this history be retained? → A: Second precision, retained indefinitely — timestamps are recorded to the nearest second, and audit history is never purged or archived within this feature's scope.
- Q: FR-008 says order history must show "relevant dates" — which specific dates/timestamps should each order display to the client? → A: Full transition timeline — the submission (Intake) timestamp plus every subsequent lifecycle transition timestamp the order has recorded (Processing, Backordered, Shipped, Final Delivery, Cancelled, as applicable).
- Q: Should this feature include explicit accessibility requirements (keyboard operability, screen-reader labeling, color-contrast) for the identity switcher, quantity controls, lifecycle indicator, and cancel/confirm controls? → A: Out of scope for this demo — no formal accessibility requirements are defined; the UI follows the reference design as-is, and accessibility can be addressed in a future feature if needed.
- Q: When a client's bulk order submission fails because contract terms are missing/expired or the computed Net Total is zero or negative, does the system persist an order record at all, or reject the request with nothing saved? → A: No record persisted — submission fails atomically; nothing is saved, the client receives an error response only, and the order never appears in history or is retrievable by ID.
- Q: If a client edits an existing order's Line Items (FR-025, while in Intake or Processing) and at that moment the contract terms are missing/expired/ambiguous, what should happen to the order? → A: Reject the edit, keep prior state — the edit is rejected with a clear reason; the order keeps its previously saved Line Items and last successfully calculated Net Total unchanged, and the client can retry the edit once contract terms are valid again.
- Q: When a request is rejected under FR-016's first-committed-wins rule, what should the rejected caller's response include so they can reconcile? → A: Reason + current status — the rejection response includes a clear conflict message plus the order's current (now-updated) lifecycle status, so the caller immediately knows what state won without a separate lookup.
- Q: Should the existing 5-second response target (SC-001, for initial order submission) also apply to Line Item edit recalculation (FR-025) and order history retrieval (FR-008)? → A: Same 5s target applies — SC-001's under-5-seconds threshold is generalized to cover edit recalculation and order history retrieval as well, since they involve comparable computation to initial submission.
- Q: If an enterprise client's order has already reached Shipped status (physically in transit, but not yet at Final Delivery), can the client still cancel it? → A: No — cancellation is only allowed while the order is in Intake, Processing, or Backordered status; once an order reaches Shipped, the client can no longer cancel it (FR-010, FR-011 revised accordingly). The FR-016 concurrency example is likewise corrected: the relevant race is between a cancellation request and an operator's advancement of the order to Shipped status, not Final Delivery.
- Q: Is a client's Contract Discount expressed as a percentage off the Gross Total, or as a fixed dollar amount? → A: Percentage — Contract Discount Terms stores a discount percentage (e.g., 12%), applied to Gross Total to compute the discount amount (Net Total = Gross Total × (1 − discount percentage)).
- Q: For SC-003's "locate the current status of any of their own orders in under 3 clicks/steps," what counts as the starting point and the completion point? → A: Start: landing on the dashboard (identity already selected). End: the order's current status is visible on the dashboard (Active Orders card or History Log entry), without navigating to a separate detail page.
- Q: Do rejection responses (invalid contract terms, invalid line items, terminal-state cancellation, etc.) need to follow a specified message format, or is any human-readable reason string sufficient? → A: Any human-readable string naming the specific failure condition (e.g., "contract terms expired," "order already shipped") is sufficient — no fixed format, error-code scheme, or localization is required.
...
2. **Given** an enterprise client whose contract discount terms are missing or expired, **When** the client submits a bulk order, **Then** the system rejects the submission and clearly indicates the reason instead of applying a default or estimated discount, and no order record is created or retrievable.
...
1. **Given** an enterprise client with multiple past and current orders, **When** the client views their order history, **Then** the system displays each order's identifier, line items, Net Total, current lifecycle status, and its full transition timeline (Intake submission timestamp plus every subsequent lifecycle transition timestamp).
...
An enterprise client cancels one of their own orders while it is still in Intake, Processing, or Backordered status (i.e., before it has shipped and it has not already been cancelled).
...
**Independent Test**: Can be fully tested by submitting an order, cancelling it before it ships, and verifying its status becomes Cancelled and it can no longer be advanced through the lifecycle.

**Acceptance Scenarios**:

1. **Given** an enterprise client's order that is in Intake, Processing, or Backordered status, **When** the client requests cancellation, **Then** the system marks the order as Cancelled and records the time of cancellation.
2. **Given** an order that has reached Shipped status, **When** the client attempts to cancel it, **Then** the system rejects the cancellation and explains that shipped orders can no longer be cancelled.
3. **Given** an order that has already reached Final Delivery, **When** the client attempts to cancel it, **Then** the system rejects the cancellation and explains that delivered orders cannot be cancelled.
...
- A client's contract discount terms change (e.g., renegotiated) while an order is still active but not yet at Final Delivery: if the order has advanced beyond Processing, its Net Total was already locked and is not recalculated; if the order is still in Intake or Processing, its Net Total is only recalculated the next time the client explicitly edits its Line Items, never automatically (see FR-017, FR-025). If, at the moment of such an edit, the client's contract terms are missing, ambiguous, or expired, the edit is rejected with a clear reason and the order retains its previously saved Line Items and last successfully calculated Net Total unchanged (see FR-025).
- A cancellation request and an operator's advancement of the order to Shipped status arrive for the same order at nearly the same moment: the request that commits first to persistent storage is applied; the other is rejected with a conflict message indicating the order's state has changed, plus the order's current lifecycle status (see FR-016).
- An enterprise client submits an order with no line items: the system rejects the submission (see FR-015). An order whose calculated Net Total is zero or negative is rejected using the same pattern as invalid contract terms (see FR-004, FR-026): no order record is created.
...
- **FR-002**: System MUST calculate a Gross Total for each submitted order as the sum of its line items' list prices and quantities, rounded to 2 decimal places (currency cents) using round-half-up rounding.
- **FR-003**: System MUST calculate the Net Total for each order by applying the submitting client's current, pre-negotiated contract discount percentage to the Gross Total (Net Total = Gross Total × (1 − discount percentage)), rounded to 2 decimal places (currency cents) using round-half-up rounding, and MUST NOT apply any manually overridden, estimated, or generic discount in place of the client's actual contract terms.
- **FR-004**: System MUST reject a submission, and MUST clearly communicate the reason, when the submitting client's contract discount terms are missing, ambiguous, or expired at the time of submission; no order record is created for a rejected submission, and it is not retrievable by any order ID.
...
- **FR-007**: System MUST record, for every lifecycle state transition and every Net Total calculation, the point in time it occurred (to the nearest second) and the actor or event that triggered it, and MUST preserve this history indefinitely (no purge or archival) rather than overwriting it.
- **FR-008**: System MUST allow an enterprise client, identified via a caller-supplied client identifier (trusted as-is; not verified by login or credentials), to view a history of their own bulk orders, including each order's line items, Gross Total, Net Total, current lifecycle status, and its full transition timeline (the Intake submission timestamp plus every subsequent lifecycle transition timestamp recorded for that order).
...
- **FR-010**: System MUST allow an enterprise client, identified via a caller-supplied client identifier (trusted as-is; not verified by login or credentials), to cancel one of their own orders only while that order is in Intake, Processing, or Backordered status; once an order reaches Shipped, Final Delivery, or Cancelled, the client can no longer cancel it.
- **FR-011**: System MUST reject a cancellation request for an order that has reached Shipped or Final Delivery status, or that has already been cancelled, and MUST communicate the specific reason for rejection (e.g., that the order has already shipped and can no longer be cancelled). For an order that does not belong to the requesting client (see FR-018), the system MUST NOT reveal that reason and MUST instead return a generic not-found response.
...
- **FR-016**: System MUST resolve concurrent conflicting requests on the same order (e.g., a cancellation request and an operator's advancement of the order to Shipped status) using first-committed-wins semantics: whichever request commits to persistent storage first is applied, and the system MUST reject the other with a clear message indicating the order's state has changed, together with the order's current (post-conflict) lifecycle status so the rejected caller can reconcile without a separate lookup.
...
- **FR-025**: System MUST allow an enterprise client to edit the Line Items (add, remove, or change quantity) of their own Bulk Order only while that order is in Intake or Processing status, and MUST recalculate the Gross Total and Net Total (per FR-002 and FR-003, using currently effective contract discount terms) on each such edit, recording the recalculation per FR-007. If, at the time of an edit, the client's contract discount terms are missing, ambiguous, or expired, the system MUST reject the edit with a clear reason and MUST leave the order's Line Items and last successfully calculated Net Total unchanged. Once an order advances beyond Processing, its Line Items and totals become read-only (see FR-017).
- **FR-026**: System MUST reject a submission, using the same rejection pattern as FR-004 (no order record created), whenever its calculated Net Total (per FR-003) is zero or negative.
...
- **Contract Discount Terms**: A single, client-specific, pre-negotiated discount terms record maintained per Enterprise Client — exactly one active record per client at any given time, with a discount percentage and a validity/expiration status — used exclusively to compute Net Total. "Current" for FR-003/FR-004 purposes means that client's one record is active and not expired at calculation time; there is no selection among multiple concurrent records.
...
- **Lifecycle Transition Record**: A record, timestamped to the nearest second, of an order moving from one lifecycle status to another, including the triggering actor or event; preserved indefinitely as history rather than overwritten or purged.
...
- **SC-001**: An enterprise client submitting a valid bulk order, editing an existing order's Line Items, or retrieving their order history receives a response (calculated Net Total, recalculated totals, or the history listing, respectively) in under 5 seconds.
...
- **SC-003**: An enterprise client can locate the current status of any of their own orders, and no other client's orders, in under 3 clicks/steps starting from landing on the dashboard with their identity already selected, ending when that order's status is visible on the dashboard (Active Orders card or History Log entry), without navigating to a separate detail page.
...
- No formal accessibility requirements (keyboard operability, screen-reader labeling, color-contrast) are defined for this feature's UI; the interface follows the reviewed reference design as-is, and accessibility can be addressed in a future feature if needed (see Clarifications, Session 2026-09-18).
- Wherever a requirement calls for a "clear message," "specific reason," or similar (e.g., FR-004, FR-011, FR-014, FR-015, FR-016, FR-024, FR-025), any human-readable string naming the specific failure condition satisfies that requirement; no fixed message format, error-code scheme, or localization is required within this feature's scope (see Clarifications, Session 2026-09-18).
