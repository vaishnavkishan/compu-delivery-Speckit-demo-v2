...
## Precision and Retention Conventions

- All monetary columns represent USD and use two fractional digits. Line subtotals
  and Gross Total are rounded to cents before the contract percentage is applied;
  Net Total is then rounded to cents using half-up rounding.
- `timestamptz(3)` is used for lifecycle, pricing, cancellation, creation, and
  update timestamps. API serialization uses UTC ISO-8601 timestamps with exactly
  millisecond precision.
- Append-only lifecycle, cancellation, and pricing records are retained for seven
  years after the order reaches Final Delivery or Cancelled. The terminal transition
  timestamp is the retention start time.
...
| `created_at`   | `timestamptz(3)` | Seed/record time in UTC.                                               |
...
| `effective_from`      | `timestamptz(3)`                  | Inclusive UTC timestamp.                    |
| `effective_until`     | `timestamptz(3)`, nullable        | Inclusive UTC timestamp; null = open-ended. |
| `created_at`          | `timestamptz(3)`                  | UTC timestamp.                              |
...
| `gross_total`                 | `numeric(12,2)`                   | USD sum of line item subtotals, rounded to cents (FR-002).                                                      |
| `applied_discount_percentage` | `numeric(5,2)`                    | Snapshot of the contract discount used for the current `net_total` (Constitution IV).                           |
| `net_total`                   | `numeric(12,2)`                   | USD `gross_total * (1 - applied_discount_percentage/100)`, rounded to cents.                                    |
| `net_total_locked_at`         | `timestamptz(3)`, nullable        | UTC timestamp set when status transitions to `SHIPPED`; once set, line items and totals are read-only (FR-017). |
| `version`                     | `bigint`                          | JPA `@Version` — optimistic lock for first-committed-wins (FR-016).                                             |
| `created_at`                  | `timestamptz(3)`                  | Intake and immediate-finalization time.                                                                         |
| `updated_at`                  | `timestamptz(3)`                  | Last mutation time, including the value returned in conflict responses.                                         |
| `cancelled_at`                | `timestamptz(3)`, nullable        | Denormalized convenience; authoritative record is `CancellationRecord`.                                         |

**Validation rules**:

- MUST have ≥1 LineItem to be created; valid creation is the finalization event and
  creates the order immediately in `INTAKE` (FR-001, FR-015).
...
| `occurred_at` | `timestamptz(3)`         | UTC millisecond precision; terminal transition starts retention.                        |
...
| `occurred_at`                 | `timestamptz(3)`        | UTC millisecond precision.                        |
...
| `cancelled_at` | `timestamptz(3)`                | UTC millisecond precision; starts retention for cancelled orders. |
